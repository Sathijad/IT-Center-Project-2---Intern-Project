...
async function hideKeyboard(driver) {
  try {
    ...
    // Keyboard might not be visible, or hideKeyboard not supported
    // Try tapping outside input fields
    try {
      ...
      await driver.pause(500);
      console.log("⌨️ Tapped to dismiss keyboard");
    } catch (e2) {
      console.log("⚠️ Could not hide keyboard, continuing...");
    }
  }
}
...
describe("Phase 7: Complete Feedback Flow", function () {
  ...
  after(async () => {
    ...
    if (driver) {
      try {
        ...
        if (e.message.includes('UND_ERR_CLOSED') || e.message.includes('invalid session id')) {
          console.log("⚠️ Session already closed");
        } else {
          console.error("⚠️ Error closing session:", e.message);
        }
      }
    }
    driver = null;
  ...
  afterEach(async function() {
    if (this.currentTest.state === 'failed') {
      try {
        const screenshotName = `./error-${this.currentTest.title.replace(/\s+/g, '-')}-${Date.now()}.png`;
  ...
  it("should complete full feedback lifecycle: create, view, add message, filter", async () => {
    try {
      ...
      // Hide keyboard before clicking button
      ...
      try {
        ...
        await submitCardContainer.waitForDisplayed({ timeout: 2000 });
        submitCardFound = true;
        console.log(`✅ Found 'Submit Feedback' card by exact content-desc`);
      } catch (e) {
        try {
          submitCardContainer = await driver.$(`//*[contains(@content-desc, "${submitCardContentDescPattern}")]`);
          await submitCardContainer.waitForDisplayed({ timeout: 2000 });
          submitCardFound = true;
          console.log(`✅ Found 'Submit Feedback' card by content-desc (contains)`);
        } catch (e2) {
          console.log("📜 Card not visible, scrolling to find it...");
        }
      }
      ...
      if (!submitCardFound) {
        for (let scroll = 0; scroll < maxScrolls; scroll++) {
          ...
          try {
            ...
            await submitCardContainer.waitForDisplayed({ timeout: 1000 });
            submitCardFound = true;
            console.log(`✅ Found 'Submit Feedback' card after scroll ${scroll + 1}`);
            break;
          } catch (e) {
            // Continue scrolling
          }
      ...
      if (!submitCardFound || !submitCardContainer) {
        throw new Error(`Could not find 'Submit Feedback' card after ${maxScrolls} scrolls.`);
      }
      ...
      // Verify we landed on the correct screen
      console.log("🔍 Verifying we landed on 'Submit Feedback' screen...");
      try {
        ...
        throw new Error("❌ Navigation failed: Not on Submit Feedback screen");
      }
      ...
      try {
        const categoryField = await driver.$('//android.widget.EditText[contains(@hint, "Category") or @hint="Category"]');
        await categoryField.waitForDisplayed({ timeout: 5000 });
        await categoryField.click();
        await driver.pause(500);
        await categoryField.clearValue();
        await categoryField.setValue("Feature Request");
        ...
        console.log("⚠️ Category field not found, continuing...");
      }
      
      // CRITICAL: Hide keyboard before scrolling and clicking submit
      console.log("⌨️ Hiding keyboard before submit...");
      await hideKeyboard(driver);
      await driver.pause(1000);
      ...
      for (let scrollAttempt = 0; scrollAttempt < 3; scrollAttempt++) {
        try {
          ...
          await driver.pause(1500);
          console.log(`✅ Scroll ${scrollAttempt + 1} completed`);
        } catch (e) {
          console.log(`⚠️ Scroll ${scrollAttempt + 1} failed:`, e.message);
        }
      }
      
      // Find Submit button with improved strategy
      console.log("📤 Finding Submit Feedback button...");
      let submitButton = null;
      ...
      const buttonSelectors = [
        ...
        '//android.widget.Button[contains(@text, "Submit")]',
        '//*[@clickable="true" and contains(@text, "Submit Feedback")]',
        '//*[@clickable="true" and contains(@text, "Submit")]',
        '//*[contains(@content-desc, "Submit Feedback")]',
      ...
      for (const selector of buttonSelectors) {
        try {
          submitButton = await driver.$(selector);
          await submitButton.waitForDisplayed({ timeout: 3000 });
          console.log(`✅ Found submit button using: ${selector.substring(0, 50)}...`);
          break;
        } catch (e) {
          continue;
        }
      ...
      if (!submitButton) {
        ...
        throw new Error("Could not find Submit Feedback button");
      }
      ...
      try {
        const location = await submitButton.getLocation();
        const size = await driver.getWindowSize();
        
        if (location.y > size.height - 200) {
          console.log("📜 Submit button not fully visible, scrolling...");
          await driver.performActions([
            {
              type: 'pointer',
              id: 'finger1',
        ...
        console.log("⚠️ Could not check button location, continuing...");
      }
      ...
      try {
        await driver.back();
        await driver.pause(2000);
        console.log("⬅️ Navigated back to home");
      } catch (e) {
        console.log("⚠️ Back navigation skipped");
      }
      ...
      try {
        ...
        await myFeedbackCardContainer.waitForDisplayed({ timeout: 2000 });
        myFeedbackCardFound = true;
        console.log(`✅ Found 'My Feedback' card`);
      } catch (e) {
        console.log("📜 Card not visible, scrolling to find it...");
        ...
        for (let scroll = 0; scroll < maxScrolls; scroll++) {
          ...
          try {
            ...
            await myFeedbackCardContainer.waitForDisplayed({ timeout: 1000 });
            myFeedbackCardFound = true;
            console.log(`✅ Found 'My Feedback' card after scroll ${scroll + 1}`);
            break;
          } catch (e2) {
            // Continue scrolling
          }
      ...
      if (!myFeedbackCardFound || !myFeedbackCardContainer) {
        throw new Error(`Could not find 'My Feedback' card`);
      }
      
      await driver.pause(1000);
      await myFeedbackCardContainer.click();
      console.log(`✅ Clicked 'My Feedback' card`);
      await driver.pause(3000);
      ...
      // === STEP 6: ADD MESSAGE TO FEEDBACK ===
      console.log("\n=== STEP 6: ADD MESSAGE ===");
      ...
      try {
        ...
        // Hide keyboard before clicking send
        await hideKeyboard(driver);
        await driver.pause(500);
      ...
      console.log("\n🎉 Complete feedback flow test finished successfully!");
      console.log("✅ All Phase 7 feedback features tested:");
      console.log("   ✓ Submit feedback");
      console.log("   ✓ View feedback list");
      console.log("   ✓ View feedback details");
      ...
      console.error("\n❌ Complete feedback flow test failed:", err.message);
